<template>
  <div class="flex items-center justify-center w-full">
    <fieldset class="flex flex-col justify-start" :aria-invalid="warn"
      :aria-describedby="warn ? question.id + 'required' : ''">
      <legend class="text-lg xl:leading-10 md:text-xl xl:text-3xl overflow-visible mb-4">{{ question.question }}
      </legend>
      <div class="flex flex-row items-baseline">
        <select
          class="block py-2 px-3 mt-3 w-full md:w-3/5 text-base md:text-lg xl:text-2xl bg-transparent rounded-md border border-solid focus:outline-none focus:ring-0 disabled:bg-gray-200 "
          :class="warn ? 'focus:border-red-500 border-red-400 bg-[#FEE2E2]' : 'focus:border-blue-400 border-zinc-400 bg-transparent'"
          :disabled="isDisabled" v-model="surveyStore.currentResponse[index].answer">
        ...
        <Transition enter-from-class="opacity-0" enter-active-class="transition-opacity duration-200 ease-linear"
          leave-to-class="opacity-0" leave-active-class="transition-opacity duration-200 ease-linear">
